...
import { searchVegetalesByCat } from '../../services/vegetales';
...
const CompradorFrutas = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await searchVegetalesByCat({ categoria: 2 });
        setFrutas(result);
      } catch (error) {
...
{frutas.map(f=>{
  return(
    ...
    <ButtonGeneric key={f.idProductoGenerico} text={f.nombreProducto} text2={`Por  ${f.tipoUnidad}`} img={peras} imgClassName="rounded-full size-14" size="landscape" className="" to={`/comprador/${f.nombreProducto}`}/>
...
//! Este es un JSON que simula la data de frutas que se consumirá del backend
// export const frutas = [
  //   // {
  //   //   "fruta": "Manzana",
  //   //   "precio": 2.5,
