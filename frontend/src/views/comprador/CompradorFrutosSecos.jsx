...
import peras from '../../assets/frutos_secos/nueces.png'
const CompradorFrutosSecos = () => {
    ...
    <section className="h-2/8  sticky top-0 text-center w-full mb-4 z-20 bg-[#F6F6F6] rounded-lg">
        <h1 className='text-3xl font-custom font-medium text-accent mb-2'>Frutos Secos</h1>
        <SearchForm/>
    </section>
...
{frutosSecos.map((fruta,index)=>{
  return(
    ...
    <ButtonGeneric key={index} text={fruta.nombre} text2={fruta.Unidad?"por unidad":"por kilo"} img={peras} imgClassName="rounded-full size-14" size="landscape" className="" to={`/comprador/${fruta.nombre}`}/>
